        ...
        <header className="mb-12">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <div className="flex items-center gap-3 mb-2">
               <Badge className="bg-primary/10 text-primary border-none rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-widest">Outbox Log</Badge>
            </div>
            <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter italic text-primary">
              ...
              <Card className="glass-card border-none rounded-[2.5rem] shadow-2xl overflow-hidden sticky top-6">
                 <CardHeader className="p-8 bg-slate-900 text-white">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <CardTitle className="text-xl font-black uppercase tracking-tighter italic">Dispatch Metrics</CardTitle>
                          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1">Institutional Outreach</p>
                       </div>
                    </div>
                 </CardHeader>
                 <CardContent className="p-8 space-y-6">
                    <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                       <p className="text-xs font-black uppercase tracking-widest text-slate-400">Total Sent</p>
                       <p className="text-2xl font-black text-slate-900">{logs.length}</p>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl">
                       <p className="text-xs font-black uppercase tracking-widest text-emerald-600">Approvals</p>
                       <p className="text-2xl font-black text-emerald-600">{logs.filter(l => l.status === 'cleared').length}</p>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-rose-50 rounded-2xl">
                       <p className="text-xs font-black uppercase tracking-widest text-rose-600">Issues Flagged</p>
                       <p className="text-2xl font-black text-rose-600">{logs.filter(l => l.status === 'issue').length}</p>
                    </div>
                 </CardContent>
              </Card>
           </div>
             ...
             <AnimatePresence>
               {logs.length === 0 ? (
                 <div className="py-20 text-center text-slate-400 font-medium flex flex-col items-center gap-4 bg-white rounded-[2.5rem] shadow-sm border border-slate-100">
                   ...
                   <p className="text-xs font-black uppercase tracking-widest">No outbound notifications yet.</p>
                 </div>
               ) : (
                 logs.map((item, i) => (
                   <motion.div
                       ...
                       <CardContent className="p-6 flex items-start gap-6">
                         ...
                         <div className="flex-1 min-w-0">
                           <div className="flex justify-between items-start mb-2">
                             <div>
                               <h4 className="text-sm font-black uppercase tracking-widest text-slate-900 truncate">
                             ...
                             <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
                           ...
                           <div className="mt-4">
                             {item.status === 'cleared' ? (
                               <Badge className="bg-emerald-500 hover:bg-emerald-600 border-none px-3 py-1 text-[9px] font-black uppercase tracking-widest">
                                 Clearance Approved
                               </Badge>
                             ) : item.status === 'issue' ? (
                               <div>
                                 <Badge className="bg-rose-500 hover:bg-rose-600 border-none px-3 py-1 text-[9px] font-black uppercase tracking-widest mb-3">
                                   Issue Flagged
                                 </Badge>
                               ...
                               <Badge className="bg-slate-900 border-none px-3 py-1 text-[9px] font-black uppercase tracking-widest">
                                 System Update
                               </Badge>
                             )}
                           </div>
