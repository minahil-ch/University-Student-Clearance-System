...
import { FileText, Plus, Trash2, Edit3, Link2, ExternalLink, ShieldCheck, GraduationCap } from "lucide-react"
...
export default function FormManagementPage() {
  ...
  const handleCreateForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error
      toast.success("Academic form generated and published to all students!")
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 font-sans">
      ...
      <main className="flex-1 lg:ml-64 p-6 md:p-10">
        <header className="mb-12">
          <h1 className="text-4xl font-black uppercase tracking-tighter">Form <span className="text-primary italic">Management</span></h1>
          ...
          <div className="xl:col-span-1 space-y-8">
            <Card className="glass-card border-none rounded-[2.5rem] shadow-2xl overflow-hidden">
              ...
              <CardContent className="p-8">
                <form onSubmit={handleCreateForm} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">External Link (Google Form/Survey)</label>
                    <Input 
                      value={newForm.link} 
            ...
            <Card className="p-8 bg-indigo-600 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
               ...
               <p className="text-indigo-100 text-xs mt-2 font-medium leading-relaxed">
                 Forms published here will immediately appear on the dashboards of all students currently registered to your department, as well as future registrations.
               </p>
            </Card>
          </div>
