    ...
    <div className="flex min-h-screen bg-blue-50/50 dark:bg-slate-950 font-sans">
      ...
      <main className="flex-1 lg:ml-64 p-6 md:p-10">
        <header className="mb-12">
          ...
          <p className="text-slate-400 font-bold font-medium text-muted-foreground text-xs mt-2">Publish Requirements to Departmental Students</p>
        </header>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-1 space-y-8">
              ...
              <CardContent className="p-8">
                <form onSubmit={handleCreateForm} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold font-medium text-muted-foreground text-slate-400 ml-1">Requirement Name</label>
                    <Input 
                      value={newForm.name} 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold font-medium text-muted-foreground text-slate-400 ml-1">External Link</label>
                    <Input 
                      value={newForm.link} 
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    <Button type="submit" className={`flex-1 h-14 text-white font-bold font-medium text-muted-foreground text-xs rounded-2xl shadow-xl ${editingFormId ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/20' : 'bg-primary hover:bg-primary/90 shadow-primary/20'}`}>
            ...
            <Card className="p-8 bg-indigo-600 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
               ...
               <h4 className="text-xl font-bold tracking-tight">Automatic Distribution</h4>
          ...
          <div className="xl:col-span-2">
            <Card className="glass-card border-none rounded-[2.5rem] shadow-2xl overflow-hidden h-full">
              <CardHeader className="p-8 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-bold tracking-tight">Active Requirements</CardTitle>
                  <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400 mt-1">Live forms visible to students</p>
                </div>
                <div className="px-4 py-1.5 bg-slate-100 rounded-full text-xs font-bold text-slate-400">
                  ...
                  {departmentForms.map((form) => (
                    <div key={form.id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center justify-between group hover:bg-white hover:shadow-xl transition-all">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <h5 className="font-bold text-slate-900 uppercase text-sm tracking-tight">{form.form_name}</h5>
                          <a href={form.form_link} target="_blank" className="text-xs font-bold text-slate-400 flex items-center gap-1 hover:text-primary transition-colors">
