    ...
    <div className="flex min-h-screen bg-[#f8fafc] dark:bg-slate-950 font-sans">
      ...
      <main className="flex-1 lg:ml-64 p-6 md:p-10">
        <header className="mb-12 flex flex-col md:flex-row justify-between items-center gap-8">
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-primary">CUI <span className="text-primary">Clearance System</span></h1>
            <p className="text-slate-400 font-bold font-medium text-muted-foreground text-xs mt-2 tracking-[0.3em]">Institutional Verification Queue</p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-8 mb-10 bg-white/40 dark:bg-slate-900/40 p-6 rounded-[3rem] border border-slate-100 dark:border-white/5 shadow-xl relative z-10">
           ...
           <div className="px-6 py-3 bg-primary/10 rounded-2xl">
              <p className="text-xs font-bold font-medium text-muted-foreground text-primary leading-none">Awaiting Review</p>
              <p className="text-xl font-bold text-primary italic mt-1">{filteredStudents.length} Requests</p>
           </div>
        </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    ...
                    <th className="px-8 py-6 text-xs font-bold tracking-wider text-slate-400 text-right">Verification Action</th>
                  </tr>
                </thead>
                      ...
                      >
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="font-bold text-slate-900 text-base leading-tight mb-1">{item.profiles?.full_name}</div>
                              <span className="text-xs font-bold font-medium text-muted-foreground text-primary bg-primary/5 px-2 py-0.5 rounded-md">{item.profiles?.reg_no}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-8 py-6">
                          <div className="space-y-1">
                             <p className="text-xs font-bold text-slate-900 uppercase">CGPA: <span className="text-emerald-600">{item.profiles?.cgpa || '0.00'}</span></p>
                             <p className="text-xs font-bold text-slate-900 uppercase truncate max-w-[150px]">Email: <span className="text-slate-500">{item.profiles?.email}</span></p>
                          </div>
                        </td>
                         <td className="px-8 py-6">
              ...
              >
                <div className="p-10">
                  <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h3 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white leading-none mb-2">
                          {selectedStudent.profiles?.full_name}
                        </h3>
                        <Badge className="bg-primary/10 text-primary border-none rounded-lg px-3 py-1 text-xs font-bold font-medium text-muted-foreground">
                  ...
                  <div className="grid grid-cols-2 gap-8 mb-10">
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400 mb-1">Father&apos;s Name</p>
                        <p className="font-bold text-slate-900 dark:text-white">{selectedStudent.profiles?.father_name || "Not specified"}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400 mb-1">Current CGPA</p>
                        <p className="font-bold text-emerald-600 text-xl tracking-tight">{selectedStudent.profiles?.cgpa || "0.00"}</p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400 mb-1">Email Address</p>
                        <p className="font-bold text-slate-900 dark:text-white truncate">{selectedStudent.profiles?.email}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400 mb-1">Phone Number</p>
                  ...
                  <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-white/5">
                     <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                           ...
                           <p className="text-xs font-bold font-medium text-muted-foreground text-slate-900 dark:text-white">Departmental Form Status</p>
                        </div>
                        <Badge className={`rounded-xl px-4 py-1.5 text-xs font-bold uppercase border-none ${selectedStudent.form_submitted ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-rose-500 text-white shadow-lg shadow-rose-500/20'}`}>
                ...
                <div className="p-8 bg-slate-900 dark:bg-black flex gap-4">
                  <Button 
                    ...
                    className="flex-1 h-14 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold uppercase text-xs tracking-widest shadow-xl shadow-emerald-500/20 disabled:opacity-30"
                  >
                    Approve Clearance
                  </Button>
                  <Button 
                    ...
                    className="flex-1 h-14 rounded-2xl bg-rose-500 hover:bg-rose-600 text-white font-bold uppercase text-xs tracking-widest shadow-xl shadow-rose-500/20"
                  >
                    Flag Issue
                  </Button>
                </div>
