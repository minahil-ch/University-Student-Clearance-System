      ...
      <main className="flex-1 w-full lg:max-w-[calc(100%-16rem)] lg:ml-64 p-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 mt-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button className="h-11 px-6 rounded-xl bg-indigo-600 hover:bg-indigo-700 shadow-xl shadow-indigo-500/20 gap-2 font-bold font-medium text-muted-foreground text-xs">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8 bg-slate-200/40 dark:bg-slate-800/40 p-1.5 rounded-[1.2rem] w-fit border border-white/10 shadow-inner">
          <button 
            ...
            className={`px-6 py-2.5 rounded-[1rem] text-xs font-bold font-medium text-muted-foreground transition-all ${activeTab === 'pending' ? 'bg-white dark:bg-slate-900 shadow-lg text-amber-500 scale-105' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Pending {pendingList.length > 0 && <span className="bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded-full ml-1 text-xs">{pendingList.length}</span>}
          </button>
          <button 
            onClick={() => { setActiveTab('cleared'); setExpandedId(null); }}
            className={`px-6 py-2.5 rounded-[1rem] text-xs font-bold font-medium text-muted-foreground transition-all ${activeTab === 'cleared' ? 'bg-white dark:bg-slate-900 shadow-lg text-emerald-500 scale-105' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Cleared {clearedList.length > 0 && <span className="bg-emerald-500/10 text-emerald-500 px-1.5 py-0.5 rounded-full ml-1 text-xs">{clearedList.length}</span>}
          </button>
          <button 
            onClick={() => { setActiveTab('future'); setExpandedId(null); }}
            className={`px-6 py-2.5 rounded-[1rem] text-xs font-bold font-medium text-muted-foreground transition-all flex items-center gap-2 ${activeTab === 'future' ? 'bg-white dark:bg-slate-900 shadow-lg text-blue-500' : 'text-slate-500 hover:text-slate-700'}`}
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 dark:bg-slate-900/80 border-b border-slate-100 dark:border-slate-800">
                    ...
                    {activeTab !== 'future' ? (
                      <>
                        <th className="px-8 py-5 text-xs font-bold tracking-wider text-slate-400">Contact Method</th>
                        <th className="px-8 py-5 text-xs font-bold tracking-wider text-slate-400">Clearance Status</th>
                        <th className="px-8 py-5 text-xs font-bold tracking-wider text-slate-400 text-center">Action</th>
                      </>
                    ) : (
                      <>
                        <th className="px-8 py-5 text-xs font-bold tracking-wider text-slate-400">Target Path</th>
                        <th className="px-8 py-5 text-xs font-bold tracking-wider text-slate-400">Institution / Org</th>
                      </>
                    )}
                  </tr>
                </thead>
                    ...
                    if (activeTab === 'future') {
                      return (
                        <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-all duration-300">
                          ...
                          <td className="px-8 py-6">
                            <span className="bg-indigo-500/10 text-indigo-500 px-3 py-1.5 rounded-xl text-xs font-bold font-medium text-muted-foreground border border-indigo-500/20">
                              {item.plan_type}
                            </span>
                          </td>
                          <td className="px-8 py-6">
                            <div className="font-bold text-xs text-slate-700 dark:text-slate-300 tracking-tight">{item.organization_name || item.university_name || 'Direct Placement'}</div>
                            <div className="text-xs text-muted-foreground mt-0.5 italic">{item.job_title || item.program_name || 'Proprietor'}</div>
                          </td>
                        </tr>
                      )
                    }
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        ...
                        >
                          <td className="px-8 py-6">
                             <div className="flex items-center gap-4">
                                <div className="w-11 h-11 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-bold shadow-inner border border-primary/20">
                                  {item.full_name?.[0] || 'S'}
                                </div>
                                <div className="space-y-0.5">
                                  <div className="font-bold text-sm text-slate-900 dark:text-white tracking-tight">{item.full_name}</div>
                                  <div className="text-xs font-bold font-medium text-muted-foreground text-primary/70">{item.reg_no}</div>
                                </div>
                             </div>
                          </td>
                          <td className="px-8 py-6">
                          ...
                          <td className="px-8 py-6">
                            <div className="flex flex-col gap-2">
                               <div className="text-xs font-bold font-medium text-muted-foreground text-slate-400">{item.clearedDepts} of {item.totalDepts} Portals OK</div>
                          ...
                          <td className="px-8 py-6">
                            <div className="flex items-center justify-center gap-4">
                              <StatusBadge status={item.isCleared ? 'cleared' : 'pending'} className="h-7 w-fit rounded-xl px-4 text-xs font-bold uppercase border-none" />
                                  ...
                                  <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
                                    ...
                                    <div className="space-y-6 xl:col-span-1">
                                       <div className="flex flex-col items-center text-center p-6 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-700">
                                         ...
                                         <h4 className="font-bold text-xl tracking-tight">{item.full_name}</h4>
                                         <p className="text-xs font-bold font-medium text-muted-foreground text-primary/60 mt-1">{item.reg_no}</p>
                                       </div>

                                       <div className="p-5 bg-rose-500/5 rounded-2xl border border-rose-500/10 space-y-4">
                                          <div className="flex items-center justify-between">
                                            <h5 className="text-xs font-bold font-medium text-muted-foreground text-rose-500">Security Credentials</h5>
                                    ...
                                    <div className="xl:col-span-2 space-y-6">
                                      <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-slate-400 border-l-3 border-primary pl-4">Verified Academic Identity</h4>
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {[
                                          ...
                                          <div key={info.label} className="p-4 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 hover:border-primary/20 transition-all">
                                            ...
                                            <div>
                                              <p className="text-xs font-bold font-medium text-muted-foreground text-slate-400">{info.label}</p>
                                    ...
                                    <div className="xl:col-span-1 space-y-4">
                                      <h4 className="text-xs font-bold uppercase tracking-[0.3em] text-slate-400 border-l-3 border-indigo-500 pl-4">Intelligence Dispatch</h4>
                                      
                                      <div className="space-y-2 p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10">
                                         <p className="text-xs font-bold text-indigo-500 font-medium text-muted-foreground mb-3">Official Communication Templates</p>
                                         <Button 
                                           onClick={(e) => { 
                                             e.stopPropagation(); 
                                           ...
                                           className="w-full text-xs h-11 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-500/20 font-bold font-medium text-muted-foreground gap-2"
                                         >
                                         ...
                                         <Button 
                                           ...
                                           className="w-full text-xs h-11 rounded-xl font-bold font-medium text-muted-foreground gap-2 text-rose-500 border-rose-500/10"
                                         >
                                         ...
                                         <Button 
                                           ...
                                           className="w-full text-xs h-11 rounded-xl font-bold font-medium text-muted-foreground gap-2 text-primary"
                                         >
              ...
              {loading && (
                <div className="py-32 flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="font-bold text-xs uppercase tracking-[0.4em] text-primary animate-pulse">Syncing Database...</p>
                </div>
              )}
              ...
              {!loading && activeDataList.length === 0 && (
                <div className="py-32 text-center flex flex-col items-center gap-6">
                  ...
                  <div>
                    <h5 className="font-bold text-slate-800 dark:text-slate-200 font-medium text-muted-foreground">No Records Discovered</h5>
